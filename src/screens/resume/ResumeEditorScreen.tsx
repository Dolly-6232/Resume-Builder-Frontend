...
const ResumeEditorScreen: React.FC = () => {
  ...
  const route = useRoute<EditorRoute>();
  const dispatch = useAppDispatch();
  const { resumes, currentResume, saving } = useAppSelector(
    state => state.resume,
  );
  ...
  const updatePersonal = (
    field: keyof NonNullable<ResumeContent['personal']>,
    value: string,
  ) => {
    setContent(prev => ({
      ...prev,
  ...
  const handleSave = async () => {
    ...
    if (
      createResume.rejected.match(action) ||
      updateResume.rejected.match(action)
    ) {
      Alert.alert(
        'Save failed',
        String(action.payload || 'Unable to save resume'),
      );
      return;
    }
    ...
    >
      ...
      >
        ...
        <Field
          label="Job title"
          value={content.personal?.role || ''}
          onChangeText={v => updatePersonal('role', v)}
          placeholder="Frontend Developer"
        />
